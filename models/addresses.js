...
const Schema = require('./schemas/addresses');
const zipTree = require('../helpers/data/zip-tree.json');

// ***** Static Methods *****
Schema.statics.create = async function(params) {
  let address = new Addresses(params);
  if (!address.city || !address.state) {
    const location = getByZipCode(address.zip);
    address.city = location ? location.city : '';
    address.state = location ? location.state : '';
  }
...
function getByLocation(lat, long, count) {
  ...
  if (locTree === null) {
    locTree = require('/server/data/loc-tree.json');
  }
